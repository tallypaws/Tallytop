...
const releases = await fetch("https://api.github.com/repos/TallyPaws/Tallytop/releases", {
    headers: {
...
const metaInfo = await (async () => {
    for (const release of releases) {
        const metaAsset = release.assets.find((a: any) => a.name === "dev.tally.Tallytop.metainfo.xml");
...
await fs.writeFile("./dist/dev.tally.Tallytop.metainfo.xml", output, "utf-8");

console.log("Updated meta information written to ./dist/dev.tally.Tallytop.metainfo.xml");
